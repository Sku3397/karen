...
Generate an appropriate SMS response:"""
    ...
    async def _handle_emergency_message(self, sms_message: SMSMessage, nlp_result) -> str:
        ...
        # Try to use emergency template first
        emergency_variables = {
            'customer_name': self._extract_customer_name(sms_message.from_number),
            'address': 'your location',  # This would come from customer profile
            'eta': '15-30 minutes'
        }
        
        template_response = self._get_template_response('emergency', emergency_variables)
        if template_response:
            emergency_response = template_response + f"\n\nCall us NOW: 757-354-4577\n\nFor life-threatening emergencies, call 911 first."
        else:
...
Response:"""
    ...
    def _get_template_response(self, intent: str, variables: Dict[str, str] = None) -> Optional[str]:
        """
        Get response using template storage system
        ...
        Returns:
            Template-based response or None if not found
        """
        ...
        intent_template_map = {
            ...
            'quote': 'quote_request_response',
            'service_inquiry': 'need_more_info',
            'general_inquiry': 'general_greeting',
            'greeting': 'general_greeting',
            'confirmation': 'yes_confirmation',
            'reschedule': 'reschedule_confirmation',
            'cancellation': 'cancellation_confirmation',
            'payment': 'payment_reminder',
            'follow_up': 'feedback_request'
        }
        ...
        if template_name:
            try:
                ...
                if template_data:
                    # Fill template with variables, using fallback if variables missing
                    template_text = template_data.get('template', '')
                    fallback_text = template_data.get('fallback', '')
                    ...
                    try:
                        ...
                        if missing_vars and fallback_text:
    ...
    def _get_fallback_response(self, intent: str) -> str:
        """
        Get fallback response if LLM and templates fail
        ...
        """
        # First try to get response from template storage
        template_response = self._get_template_response(intent)
        if template_response:
            return template_response
            
        # If template not found, use hard-coded fallbacks
        fallback_responses = {
    ...
    def send_template_sms(self, to_number: str, template_name: str, variables: Dict[str, str] = None, priority: str = 'normal') -> SMSMessage:
        """
        Send SMS using a template from storage
        ...
        try:
            ...
            if missing_vars and fallback_text:
                ...
                for var in missing_vars:
                    template_text = template_text.replace(f"{{{var}}}", "")
                message = re.sub(r'\s+', ' ', template_text).strip()
            else:
                message = template_text
            
            # Send the SMS
            return self.send_sms(to_number, message, priority)
            
        except Exception as e:
            logger.error(f"Error sending template SMS '{template_name}': {e}")
            # Fall back to a basic message
            fallback_message = f"Thank you for contacting 757 Handy. We'll respond shortly."
            return self.send_sms(to_number, fallback_message, priority)
    ...
    def _extract_customer_name(self, phone_number: str) -> str:
        """
        Extract customer name from phone number using customer profiles
        
        Args:
            phone_number: Customer's phone number
            
        Returns:
            Customer name or empty string if not found
        """
        formatted_number = self._format_phone_number(phone_number)
        customer_profile = self.customer_profiles.get(formatted_number, {})
        return customer_profile.get('name', '')
