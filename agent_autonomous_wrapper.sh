...
execute_task() {
    ...
    # --- BEGIN MODIFIED SECTION FOR REAL WORK ---
    task_id=$(grep "## Task ID:" "$processing_file" | sed 's/## Task ID: //' | xargs)
    # Attempt to get target_file from ## Source File: line if it exists
    target_file_from_md=$(grep "## Source File:" "$processing_file" | sed 's/## Source File: //' | xargs)
    description_line=$(sed -n '/## Task Description/,/## Expected Deliverables/{/## Task Description/n;/## Expected Deliverables/!p;}' "$processing_file" | head -n 1 | xargs)

    log DEBUG "Extracted Task ID for routing: [$task_id]"
    log DEBUG "Extracted Target File from MD: [$target_file_from_md]"
    log DEBUG "Extracted Description for Python: [$description_line]"

    PYTHON_DEV_SCRIPT="$PROJECT_ROOT/src/nlp_tasks.py"
    ACTUAL_TARGET_FILE=""
    PERFORM_REAL_WORK=false

    # Specific routing for EGT_KAREN_NLP_001
    if [ "$AGENT_NAME" == "sms_engineer" ] && [ "$task_id" == "EGT_KAREN_NLP_001" ]; then
        log INFO "Task EGT_KAREN_NLP_001 detected for sms_engineer. Attempting real work."
        # If target_file_from_md is empty or invalid, hardcode for this specific task for now
        if [ -z "$target_file_from_md" ] || [ "$target_file_from_md" == "N/A" ]; then
            ACTUAL_TARGET_FILE="$PROJECT_ROOT/src/nlp/sentiment_analyzer.py"
            log WARN "Source File not found in .md, using default for EGT_KAREN_NLP_001: $ACTUAL_TARGET_FILE"
        else
            ACTUAL_TARGET_FILE="$PROJECT_ROOT/$target_file_from_md" # Assuming relative path from project root
        fi
        log INFO "Final Target File for Python: $ACTUAL_TARGET_FILE"
        PERFORM_REAL_WORK=true
    fi

    task_successful=false
    if [ "$PERFORM_REAL_WORK" = true ]; then
        if [ -f "$PYTHON_DEV_SCRIPT" ] && [ -f "$ACTUAL_TARGET_FILE" ]; then
            log INFO "Executing Python script: python $PYTHON_DEV_SCRIPT --task_id '$task_id' --target_file '$ACTUAL_TARGET_FILE' --description '$description_line'"
            if python "$PYTHON_DEV_SCRIPT" --task_id "$task_id" --target_file "$ACTUAL_TARGET_FILE" --description "$description_line"; then
                log SUCCESS "Python development script for task $task_id SUCCESSFUL."
                task_successful=true
            else
                log ERROR "Python development script for task $task_id FAILED with exit code $?."
                task_successful=false
            fi
        else
            log ERROR "Python script $PYTHON_DEV_SCRIPT or target file $ACTUAL_TARGET_FILE not found for task $task_id."
            task_successful=false
        fi
    else
        log INFO "No real work defined for Task ID [$task_id] and Agent [$AGENT_NAME]. Performing SIMULATED work."
        # Original simulation logic (fallback)
        local work_duration=$((RANDOM % 10 + 5)) # Shortened simulation for testing: 5-14 seconds
        log DEBUG "Simulating work for $work_duration seconds..."
        for ((i=1; i<=work_duration; i++)); do
            sleep 1
            log DEBUG "Simulated task progress: $i/$work_duration seconds"
        done
        task_successful=true # Simulation is always successful for this test
    fi
    # --- END MODIFIED SECTION FOR REAL WORK ---

    local task_start_time=$(stat -c %Y "$processing_file" 2>/dev/null || date +%s) # Approx start
    local completion_time=$(date +%s)
    local duration=$((completion_time - task_start_time))

    if [ "$task_successful" = true ]; then
        log SUCCESS "Task $task_id completed in $duration seconds."
        ...
        log ERROR "Task $task_id failed in $duration seconds."
        # Move failed task to error directory
        mkdir -p "$AGENT_PROGRESS_DIR/failed"
        mv "$processing_file" "$AGENT_PROGRESS_DIR/failed/failed_$(basename "$processing_file")_$(date +%Y%m%d_%H%M%S)"
    fi
