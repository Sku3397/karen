...
class AutonomousAgentRunner:
    ...
    def get_next_task(self, agent_name: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Error decoding JSON from task file {self.task_file}: {e}")
            return None
        except Exception as e:
            logger.error(f"Error reading task file {self.task_file}: {e}")
            return None
        ...
        for task in tasks:
            if not isinstance(task, dict):
                logger.warning(f"Skipping invalid task item (not a dict): {task}")
                continue
            ...
            if not task_id:
                logger.warning(f"Skipping task due to missing 'id': {task.get('description', 'No description')[:50]}...")
                continue

            if status != 'pending':
                # This is normal, just means task is not available for picking up
                # logger.debug(f"Task {task_id} is not pending (status: {status}). Skipping.")
                continue

            # If we are here, task has an ID and is pending.
            # Now check for skill match or general low priority task.
            task_desc = task.get('description', '').lower()
            assigned_to = task.get('assigned_to') # Check if it's explicitly assigned

            # Prefer tasks specifically matching skills and not yet assigned OR assigned to this agent type
            if any(skill in task_desc for skill in skills):
                if not assigned_to or assigned_to == agent_name or assigned_to == 'EigencodeAgent': # EigencodeAgent can be a general pool
                    logger.info(f"Found skill-matching pending task {task_id} for {agent_name}.")
                    return task
            
            # Fallback: general low priority tasks that are not assigned to anyone specific
            if task.get('priority') == 'low' and not assigned_to:
                logger.info(f"Found general low-priority pending task {task_id} for {agent_name}.")
                return task
...
## Status: Starting
## Source File: {task.get('source_file', 'N/A')}
